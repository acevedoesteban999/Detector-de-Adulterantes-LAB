...
class TrainingingForm(forms.ModelForm):
    ...
    class Meta:
        model =Training
        fields = 'name','count', 'predict',
        widgets = {
            'name': forms.TextInput(attrs={'class':'form-control','placeholder': 'Ingrese un nombre'}),
            'count': forms.NumberInput(attrs={'class':'form-control','placeholder': 'Ingrese cantidad de muestras'}),
            'predict': forms.Select(attrs={'class':'form-control'}),
        }
    ...
    def save(self,multi=False):
        ...
        if not thread_is_alive("ThreadTrainings"):
            Thread(target=train_thread,name="ThreadTrainings",args=(name,self.cleaned_data.get('count'),self.cleaned_data.get('predict'),)).start()
            return True
        return False
...
class CSVForm(forms.Form):
    ...
    count=forms.IntegerField(required=False,label="Cantidad",widget=forms.NumberInput(attrs={'class':'form-control','placeholder': 'Ingrese cantidad datos a cargar '}))
